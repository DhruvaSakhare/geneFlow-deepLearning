...
Download the Virtual Cell Challenge `training.h5ad` file and place it under `data/`.

### Split into train / val / test

Held out by perturbation identity (so test perturbations are never seen during training):
...
Produces `data/training.h5ad`, `data/validation.h5ad`, `data/test.h5ad` (the input file is overwritten by the new training-only split).

### Update config

Edit `perturbation_fm/configs/default.yaml`:

```yaml
train_path: data/training.h5ad
val_path:   data/validation.h5ad
emb_path:   embeddings/esm2_embeddings.pt
